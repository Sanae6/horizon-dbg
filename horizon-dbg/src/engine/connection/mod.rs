...
use horizon_dbg_protocol::{frame::Header, postcard::{self, to_stdvec}, postcard_schema::Schema, serde::Serialize, subscription::{DeleteSubscription, SubscriptionUpdated, UpdateSubscription, DELETE_SUBSCRIPTION, UPDATE_SUBSCRIPTION}, Keygen};
use postcard_rpc::{Key, Key2};
use tokio::{
...
impl Connection {
  ...
  fn write_message<T: Schema + Serialize + Send + 'static>(&mut self, key: &'static Keygen, value: T) {
    let length =
  ...
  pub fn update_subscription_address(&mut self, index: u64, address: u64, size: u32) {
    self.write_message(&UPDATE_SUBSCRIPTION, UpdateSubscription {
      id: index,
